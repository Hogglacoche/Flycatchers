...
## coll_dn_ds_gc_L
output_file <- file.path(output_directory, "resultats_coll_dn_ds_gc_L.txt")
...
## coll_dn_ds_gc_S
output_file <- file.path(output_directory, "resultats_coll_dn_ds_gc_S.txt")
...
## coll_dn_ds_all_L
output_file <- file.path(output_directory, "resultats_coll_dn_ds_all_L.txt")
...
## coll_dn_ds_all_S
output_file <- file.path(output_directory, "resultats_coll_dn_ds_all_S.txt")
...
## taiga_dn_ds_gc_S
output_file <- file.path(output_directory, "resultats_taiga_dn_ds_gc_S.txt")
...
## taiga_dn_ds_all.S
output_file <- file.path(output_directory, "resultats_taiga_dn_ds_all_S.txt")
...
pi_table_taiga<- pi_table_taiga[, c('chr', 'dn_ds_gene_gc_S', 'dn_ds_gene_all_S')]
pi_table_taiga<-na.omit(pi_table_taiga)
pi_table_taiga[pi_table_taiga == "NULL"] <- 0
pi_table_taiga$dn_ds_gene_gc_S<- sapply(pi_table_taiga$dn_ds_gene_gc_S, as.numeric)
pi_table_taiga$dn_ds_gene_gc_S-as.numeric(pi_table_taiga$dn_ds_gene_gc_S)
pi_table_taiga$dn_ds_gene_all_S<- sapply(pi_table_taiga$dn_ds_gene_all_S, as.numeric)
pi_table_taiga$dn_ds_gene_all_S<-as.numeric(pi_table_taiga$dn_ds_gene_all_S)
...
for (chr in names(groupes)) {
  dn_ds_gene_gc_S <- as.numeric(groupes[[chr]][[2]]) 
  dn_ds_gene_all_S <- as.numeric(groupes[[chr]][[3]])  
    cor_chr <- cor(dn_ds_gene_gc_S, dn_ds_gene_all_S)
...
ggplot(resultats_df, aes(x = Chromosome, y = Correlation)) +
  ...
  labs(title = "Correlation between dN/dS GC and dN/dS all sites (short branch) for each chromosome (Ficedula taiga) ",
       x = "Chromosome",
       y = "Correlation") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) 
...
###################################
#correlate chr
# albicollis dn/ds all GC

setwd("/home/amaniouloux/Documents/data/dataframe//")

pi_table_albicollis <- read.table("table_albicollis.txt", header = T)
pi_table_albicollis<- pi_table_albicollis[, c('chr', 'dn_ds_gene_gc_S', 'dn_ds_gene_all_S')]
pi_table_albicollis<-na.omit(pi_table_albicollis)
pi_table_albicollis[pi_table_albicollis == "NULL"] <- 0
pi_table_albicollis$dn_ds_gene_gc_S<- sapply(pi_table_albicollis$dn_ds_gene_gc_S, as.numeric)
pi_table_albicollis$dn_ds_gene_gc_S-as.numeric(pi_table_albicollis$dn_ds_gene_gc_S)
pi_table_albicollis$dn_ds_gene_all_S<- sapply(pi_table_albicollis$dn_ds_gene_all_S, as.numeric)
pi_table_albicollis$dn_ds_gene_all_S<-as.numeric(pi_table_albicollis$dn_ds_gene_all_S)
...
index_a_retirer <- which(is.na(resultats_corr))
resultats_corr <- resultats_corr[-index_a_retirer]
resultats_corr_ord<- resultats_corr
...
ggplot(resultats_df, aes(x = Chromosome, y = Correlation)) +
  geom_point(color = "black", size = 3) +  # Ajouter les points
  geom_smooth(method = "lm", se = FALSE) + 
  labs(title = "Correlation between dN/dS GC and dN/dS all sites (short branch) for each chromosome (Ficedula albicollis) ",
       x = "Chromosome",
       y = "Correlation") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) 

##################
# correlate dn/ds taiga dn/ds albi GC

dnds_table_albicollis <- read.table("table_albicollis.txt", header = T)
dnds_table_albicollis<- dnds_table_albicollis[, c('gene', 'chr', 'dn_ds_gene_gc_S', 'dn_ds_gene_all_S')]
dnds_table_albicollis<-na.omit(dnds_table_albicollis)
dnds_table_albicollis[dnds_table_albicollis == "NULL"] <- 0
dnds_table_albicollis$dn_ds_gene_gc_S<- sapply(dnds_table_albicollis$dn_ds_gene_gc_S, as.numeric)
dnds_table_albicollis$dn_ds_gene_gc_S-as.numeric(dnds_table_albicollis$dn_ds_gene_gc_S)
dnds_table_albicollis$dn_ds_gene_all_S<- sapply(dnds_table_albicollis$dn_ds_gene_all_S, as.numeric)
dnds_table_albicollis$dn_ds_gene_all_S<-as.numeric(dnds_table_albicollis$dn_ds_gene_all_S)

dnds_table_taiga <- read.table("table_taiga.txt", header = T)
dnds_table_taiga<- dnds_table_taiga[, c('gene', 'chr', 'dn_ds_gene_gc_S', 'dn_ds_gene_all_S')]
dnds_table_taiga<-na.omit(dnds_table_taiga)
dnds_table_taiga[dnds_table_taiga == "NULL"] <- 0
dnds_table_taiga$dn_ds_gene_gc_S<- sapply(dnds_table_taiga$dn_ds_gene_gc_S, as.numeric)
dnds_table_taiga$dn_ds_gene_gc_S-as.numeric(dnds_table_taiga$dn_ds_gene_gc_S)
dnds_table_taiga$dn_ds_gene_all_S<- sapply(dnds_table_taiga$dn_ds_gene_all_S, as.numeric)
dnds_table_taiga$dn_ds_gene_all_S<-as.numeric(dnds_table_taiga$dn_ds_gene_all_S)

dnds_table <- merge(dnds_table_albicollis, dnds_table_taiga, by = 'gene', all = TRUE)
...
dnds_table <- dnds_table[mixedorder(dnds_table$chr), ]
groupes <- split(dnds_table, dnds_table$chr)

resultats_corr <- list()

for (chr in names(groupes)) {
  dn_ds_gene_gc_S_coll <- as.numeric(groupes[[chr]][[2]]) 
  dn_ds_gene_gc_S_taiga <- as.numeric(groupes[[chr]][[4]])  
  cor_chr <- cor(dn_ds_gene_gc_S_coll, dn_ds_gene_gc_S_taiga)
  resultats_corr[[chr]] <- cor_chr
}
#ALL sites
#for (chr in names(groupes)) {
  dn_ds_gene_all_S_coll <- as.numeric(groupes[[chr]][[3]]) 
  dn_ds_gene_all_S_taiga <- as.numeric(groupes[[chr]][[5]])  
  cor_chr <- cor(dn_ds_gene_all_S_coll, dn_ds_gene_all_S_taiga)
  resultats_corr[[chr]] <- cor_chr
#}
...
resultats_corr <- resultats_corr_ord
...
ggplot(resultats_df, aes(x = Chromosome, y = Correlation)) +
  ...
  geom_smooth(method = "lm", se = FALSE) + 
  labs(title = "Correlation dN/dS GC (short branch) between Ficedula albicollis and Ficedula taiga for each chromosome ",
       x = "Chromosome",
       y = "Correlation") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) 
...
coll_dn_ds_gc_L_macro <- aggregate(cbind(dn_tot_gc_L, dn_norm_tot_gc_L, ds_tot_gc_L, ds_norm_tot_gc_L) ~ category == "Macro", table_albicollis_macro, sum)
...
coll_dn_ds_gc_L_inter <- aggregate(cbind(dn_tot_gc_L, dn_norm_tot_gc_L, ds_tot_gc_L, ds_norm_tot_gc_L) ~ category == "Intermediate", table_albicollis_inter, sum)
...
coll_dn_ds_gc_L_micro <- aggregate(cbind(dn_tot_gc_L, dn_norm_tot_gc_L, ds_tot_gc_L, ds_norm_tot_gc_L) ~ category == "Micro", table_albicollis_micro, sum)
...
table_albicollis_macro <- subset(table_albicollis, category == "Macro")
table_albicollis_macro<-na.omit(table_albicollis_macro)
coll_dn_ds_gc_S_macro <- aggregate(cbind(dn_tot_gc_S, dn_norm_tot_gc_S, ds_tot_gc_S, ds_norm_tot_gc_S) ~ category == "Macro", table_albicollis_macro, sum)
...
table_albicollis_inter <- subset(table_albicollis, category == "Intermediate")
table_albicollis_inter<-na.omit(table_albicollis_inter)
coll_dn_ds_gc_S_inter <- aggregate(cbind(dn_tot_gc_S, dn_norm_tot_gc_S, ds_tot_gc_S, ds_norm_tot_gc_S) ~ category == "Intermediate" , table_albicollis_inter, sum)
...
table_albicollis_micro <- subset(table_albicollis, category == "Micro")
table_albicollis_micro<-na.omit(table_albicollis_micro)
coll_dn_ds_gc_S_micro <- aggregate(cbind(dn_tot_gc_S, dn_norm_tot_gc_S, ds_tot_gc_S, ds_norm_tot_gc_S) ~ category == "Micro", table_albicollis_micro, sum)
...
table_albicollis_macro <- subset(table_albicollis, category == "Macro")
table_albicollis_macro<-na.omit(table_albicollis_macro)
coll_dn_ds_all_L_macro <- aggregate(cbind(dn_tot_all_L, dn_norm_tot_all_L, ds_tot_all_L, ds_norm_tot_all_L) ~ category == "Macro", table_albicollis_macro, sum)
...
table_albicollis_inter <- subset(table_albicollis, category == "Intermediate")
table_albicollis_inter<-na.omit(table_albicollis_inter)
coll_dn_ds_all_L_inter <- aggregate(cbind(dn_tot_all_L, dn_norm_tot_all_L, ds_tot_all_L, ds_norm_tot_all_L) ~ category == "Intermediate", table_albicollis_inter, sum)
...
table_albicollis_micro <- subset(table_albicollis, category == "Micro")
table_albicollis_micro<-na.omit(table_albicollis_micro)
coll_dn_ds_all_L_micro <- aggregate(cbind(dn_tot_all_L, dn_norm_tot_all_L, ds_tot_all_L, ds_norm_tot_all_L) ~ category == "Micro", table_albicollis_micro, sum)
...
table_albicollis_macro <- subset(table_albicollis, category == "Macro")
table_albicollis_macro<-na.omit(table_albicollis_macro)
coll_dn_ds_all_S_macro <- aggregate(cbind(dn_tot_all_S, dn_norm_tot_all_S, ds_tot_all_S, ds_norm_tot_all_S) ~ category == "Macro", table_albicollis_macro, sum)
...
table_albicollis_inter <- subset(table_albicollis, category == "Intermediate")
table_albicollis_inter<-na.omit(table_albicollis_inter)
coll_dn_ds_all_S_inter <- aggregate(cbind(dn_tot_all_S, dn_norm_tot_all_S, ds_tot_all_S, ds_norm_tot_all_S) ~ category == "Intermediate" , table_albicollis_inter, sum)
...
table_albicollis_micro <- subset(table_albicollis, category == "Micro")
table_albicollis_micro<-na.omit(table_albicollis_micro)
coll_dn_ds_all_S_micro <- aggregate(cbind(dn_tot_all_S, dn_norm_tot_all_S, ds_tot_all_S, ds_norm_tot_all_S) ~ category == "Micro", table_albicollis_micro, sum)
...
taiga_dn_ds_all_S_macro <- aggregate(cbind(dn_tot_all_S, dn_norm_tot_all_S, ds_tot_all_S, ds_norm_tot_all_S) ~ category == "Macro", table_taiga_macro, sum)
...
taiga_dn_ds_all_S_inter <- aggregate(cbind(dn_tot_all_S, dn_norm_tot_all_S, ds_tot_all_S, ds_norm_tot_all_S) ~ category == "Intermediate" , table_taiga_inter, sum)
...
taiga_dn_ds_all_S_micro <- aggregate(cbind(dn_tot_all_S, dn_norm_tot_all_S, ds_tot_all_S, ds_norm_tot_all_S) ~ category == "Micro", table_taiga_micro, sum)
...
table_taiga_macro <- subset(table_taiga, category == "Macro")
table_taiga_macro<-na.omit(table_taiga_macro)
taiga_dn_ds_gc_S_macro <- aggregate(cbind(dn_tot_gc_S, dn_norm_tot_gc_S, ds_tot_gc_S, ds_norm_tot_gc_S) ~ category == "Macro", table_taiga_macro, sum)
...
taiga_dn_ds_gc_S_macro$coll_dn_ds_gc_S <- with(taiga_dn_ds_gc_S_macro, calculer_dn_ds(dn_tot_gc_S, dn_norm_tot_gc_S, ds_tot_gc_S, ds_norm_tot_gc_S))
taiga_dn_ds_gc_S_macro<-mean(taiga_dn_ds_gc_S_macro$coll_dn_ds_gc_S)

table_taiga_inter <- subset(table_taiga, category == "Intermediate")
table_taiga_inter<-na.omit(table_taiga_inter)
taiga_dn_ds_gc_S_inter <- aggregate(cbind(dn_tot_gc_S, dn_norm_tot_gc_S, ds_tot_gc_S, ds_norm_tot_gc_S) ~ category == "Intermediate" , table_taiga_inter, sum)
...
taiga_dn_ds_gc_S_inter$coll_dn_ds_gc_S <- with(taiga_dn_ds_gc_S_inter, calculer_dn_ds(dn_tot_gc_S, dn_norm_tot_gc_S, ds_tot_gc_S, ds_norm_tot_gc_S))
taiga_dn_ds_gc_S_inter<-mean(taiga_dn_ds_gc_S_inter$coll_dn_ds_gc_S)

table_taiga_micro <- subset(table_taiga, category == "Micro")
table_taiga_micro<-na.omit(table_taiga_micro)
taiga_dn_ds_gc_S_micro <- aggregate(cbind(dn_tot_gc_S, dn_norm_tot_gc_S, ds_tot_gc_S, ds_norm_tot_gc_S) ~ category == "Micro", table_taiga_micro, sum)
...
taiga_dn_ds_gc_S_micro$coll_dn_ds_gc_S <- with(taiga_dn_ds_gc_S_micro, calculer_dn_ds(dn_tot_gc_S, dn_norm_tot_gc_S, ds_tot_gc_S, ds_norm_tot_gc_S))
taiga_dn_ds_gc_S_micro<-mean(taiga_dn_ds_gc_S_micro$coll_dn_ds_gc_S)
...
merged_dn_ds<-merge(merged_dn_ds, size, by="chr", all =T)
merged_dn_ds<-na.omit(merged_dn_ds)
categories <- c("Macro", "Micro")

merged_dn_ds_sum <- aggregate(cbind(coll_dn_ds_all_L, coll_dn_ds_all_S, coll_dn_ds_gc_L, coll_dn_ds_gc_S, taiga_dn_ds_all_S, taiga_dn_ds_gc_S) ~ category, merged_dn_ds, sum)
merged_dn_ds_mean <- aggregate(cbind(coll_dn_ds_all_L, coll_dn_ds_all_S, coll_dn_ds_gc_L, coll_dn_ds_gc_S, taiga_dn_ds_all_S, taiga_dn_ds_gc_S) ~ new_category, merged_dn_ds, mean)
merged_dn_ds_mean$new_category <- factor(merged_dn_ds_mean$new_category, levels = c("Macro", "Micro"))
library(ggplot2)
palette_couleurs <- c("coll dn/ds GC L" = "steelblue4", 
                      "coll dn/ds GC S" = "steelblue1",
                      "coll dn/ds ALL L" = "tomato3",
